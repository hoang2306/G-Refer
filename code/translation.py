...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", type=str, required=True, help="Dataset to use (e.g., yelp)"
    )
    parser.add_argument(
        "--split", type=str, required=True, help="Data split to use (e.g., trn)"
    )
    parser.add_argument(
        "--text_encoder",
...
def flatten_pagelink_retrieval_results(pagelink_retrieval_results, mapper, k):
    ...
    for key, value in pagelink_retrieval_results.items():
        ...
        user_id = key[0][1]
        item_id = key[1][1]
        flattened_results[(user_id, item_id)] = (
            prompt_template + " " + " ".join(path_prompts)
        )
...
def flatten_dense_retrieval_results(dense_retrieval_results, mapper, k):
    ...
    for key, value in dense_retrieval_results.items():
        ...
        concatenated_texts = prompt_template.format(
            ", ".join(topk_user_texts), ", ".join(topk_item_texts)
        )
        flattened_results[user_item_tuple] = concatenated_texts
    return flattened_results


def merge_flattened_results(
    flattened_dense_retrieval_results, flattened_pagelink_retrieval_results
):
...
def sample_generation(args, merged_results, dict_data, pyg_data):
    ...
    for i in range(len(dict_data["uid"])):
        ...
        if pyg_data.user_id_to_node.get(
            dict_data["uid"][i]
        ) and pyg_data.item_id_to_node.get(dict_data["iid"][i]):
            ui_pair = tuple(
                (
                    pyg_data.user_id_to_node.get(dict_data["uid"][i]),
                    pyg_data.item_id_to_node.get(dict_data["iid"][i])
                    - len(pyg_data.user_id_to_node),
                )
            )
        else:
            ui_pair = None
...
def rerank_flattened_results(args, all_samples, mapper, write_file):
    ...
    for i in tqdm(range(0, len(all_samples), batch_size), desc="Reranking samples"):
        ...
        combined_texts = [
            ...
            for user_text, item_text in zip(user_texts, item_texts)
        ]
...
def main():
    ...
    mapper = DataMapper(f"data/{args.dataset}/data_{args.split}.pt", debug_mode=args.debug)

    # load dense retrieval results
    with open(
        f"data/{args.dataset}/dense_retrieval_results_{args.split}.json", "r"
    ) as f:
    ...
    flattened_dense_retrieval_results = flatten_dense_retrieval_results(
        dense_retrieval_results, mapper, args.k
    )
    flattened_pagelink_retrieval_results = flatten_pagelink_retrieval_results(
        pagelink_retrieval_results, mapper, args.k
    )
