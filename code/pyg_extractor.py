...
def count_users_items(subgraph_nodes, pyg_data):
    user_mask = torch.isin(
        subgraph_nodes, torch.tensor(list(pyg_data.user_id_to_node.values()))
    )
...
def spectral_clustering(data, n_clusters):
    ...
    similarity_matrix = np.exp(
        -(squareform(distances) ** 2) / (2.0 * np.std(distances) ** 2)
    )
...
def process_data(pyg_data, pkl_data, k, n_clusters):
    ...
    sentence_model = sentence_model.to(
        torch.device("cuda" if torch.cuda.is_available() else "cpu")
    )

    # Create a progress bar
    progress_bar = tqdm(
        pkl_data.iterrows(), total=len(pkl_data), desc="Processing data"
    )
    ...
    for _, row in progress_bar:
        ...
        if user in pyg_data.user_id_to_node and item in pyg_data.item_id_to_node:
            ...
            subgraph_nodes, subgraph_edge_index = extract_k_hop_subgraph(
                pyg_data, user_node, item_node, k
            )
            user_count, item_count = count_users_items(subgraph_nodes, pyg_data)

            # Perform community detection
            subgraph_data = Data(
                x=pyg_data.x[subgraph_nodes], edge_index=subgraph_edge_index
            )
            ...
            cluster_embeddings = [
                pyg_data.x[subgraph_nodes[community_labels == i]]
                .mean(dim=0)
                .cpu()
                .numpy()
            ...
            similarities = cosine_similarity(
                explanation_embedding.reshape(1, -1), cluster_embeddings
            )[0]
            ...
            progress_bar.set_postfix(
                user=user, item=item, users=user_count, items=item_count
            )
