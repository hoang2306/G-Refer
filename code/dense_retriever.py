...
class DenseRetriever(nn.Module):
    ...
    def retrieve_topk(self, pyg_data, pkl_data, data_mapper, topk):
        ...
        progress_bar = tqdm(
            pkl_data.iterrows(), total=len(pkl_data), desc="Dense Retrieval"
        )
        ...
        for _, row in progress_bar:
            ...
            _, subgraph_edge_index = extract_k_hop_subgraph(
                pyg_data, user_idx, item_idx, 2
            )
            ...
            subgraph_edge_index = subgraph_edge_index[
                :,
                (subgraph_edge_index[0] < num_users)
                | (subgraph_edge_index[1] >= num_users),
            ]
            users_who_bought_item = set(
                subgraph_edge_index[0][subgraph_edge_index[1] == item_idx].tolist()
            )
            users_who_bought_item.discard(user_idx)
            user_similarities = [
                (
                    other_user,
                    self.cosine_similarity(user_feat, pyg_data.x[other_user]).item(),
                )
                for other_user in users_who_bought_item
            ]
            topk_users = sorted(user_similarities, key=lambda x: x[1], reverse=True)[
                :topk
            ]
            ...
            items_bought_by_user = set(
                subgraph_edge_index[1][subgraph_edge_index[0] == user_idx].tolist()
            )
            items_bought_by_user.discard(item_idx)
            item_similarities = [
                (
                    other_item,
                    self.cosine_similarity(item_feat, pyg_data.x[other_item]).item(),
                )
                for other_item in items_bought_by_user
            ]
            topk_items = sorted(item_similarities, key=lambda x: x[1], reverse=True)[
                :topk
            ]
            ...
            retrieval_results[f"{user_idx}-{item_idx}"] = {
                ...
                "topk_user_raw_texts": [
                    data_mapper.get_user_raw_text(user) for user in topk_users
                ],
                "topk_item_raw_texts": [
                    data_mapper.get_item_raw_text(item - data_mapper.num_users)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Retrieve top-k similar users and items"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--topk", type=int, default=5, help="Number of top-k similar nodes to retrieve"
    )
    parser.add_argument(
        "--pruning_score", type=float, default=0, help="Pruning score for similarity"
    )
...
def main():
    ...
    data_mapper = DataMapper(
        pt_file_path=f"data/{args.dataset}/data_{args.split}.pt", debug_mode=args.debug
    )

    retriever = DenseRetriever(args.pruning_score)
    retrieval_results = retriever.retrieve_topk(
        pyg_data, pkl_data, data_mapper, args.topk
    )

    # Save retrieval results
    with open(
        f"data/{args.dataset}/dense_retrieval_results_{args.split}.json", "w"
    ) as f:
        json.dump(retrieval_results, f)

    print(
        f"Retrieval results saved to data/{args.dataset}/dense_retrieval_results_{args.split}.json"
    )
