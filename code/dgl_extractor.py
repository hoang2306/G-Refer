...
def create_dgl_graph(pyg_data, pkl_data, debug=False):
    ...
    likes_edges = set()
    for _, row in tqdm(
        pkl_data.iterrows(), desc="Processing pkl_data", total=len(pkl_data)
    ):
        if (
            row["uid"] in pyg_data.user_id_to_node
            and row["iid"] in pyg_data.item_id_to_node
        ):
    ...
    # buys_edges -= likes_edges # remove to train :D 
    ...
    g.add_edges(
        buys_src, buys_dst, etype="have_bought"
    )  # replace 'buys' with 'have_bought'
...
def main():
    ...
    dgl.save_graphs(
        f"./PaGE-Link/datasets/{args.dataset}_{args.split}.bin", [dgl_graph]
    )
