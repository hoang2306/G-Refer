...
parser.add_argument(
    "--lr", type=float, default=0.01, help="link predictor learning_rate"
)
parser.add_argument(
    "--num_epochs", type=int, default=200, help="How many epochs to train"
)
parser.add_argument(
    "--eval_interval", type=int, default=1, help="Evaluate once per how many epochs"
)
parser.add_argument(
    "--save_model", default=False, action="store_true", help="Whether to save the model"
)
parser.add_argument(
    "--saved_model_dir",
    type=str,
...
def run():
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if args.sample_neg_edges:
            train_neg_src_nids, train_neg_tgt_nids = negative_sampling(
                train_pos_g, pred_etype
            )
        ...
        if epoch % args.eval_interval == 0:
            with torch.no_grad():
                ...
                print(
                    "In epoch {}, loss: {:.4f}, train AUC: {:.4f}, val AUC: {:.4f}".format(
    ...
    with torch.no_grad():
        ...
        print(
            "Best epoch {}, val AUC: {:.4f}, test AUC: {:.4f}".format(
...
if args.save_model:
    ...
    torch.save(
        model.state_dict(),
        output_dir.joinpath(f"{args.dataset_name}_model_{args.split}.pth"),
    )
