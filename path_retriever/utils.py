...
def get_ntype_hetero_nids_to_homo_nids(ghetero, debug=False):
    """
...
def get_ntype_pairs_to_cannonical_etypes(ghetero, pred_etype="likes"):
    ...
    for src_ntype, etype, tgt_ntype in ghetero.canonical_etypes:
        if etype != pred_etype:
            ntype_pairs_to_cannonical_etypes[(src_ntype, tgt_ntype)] = (
                src_ntype,
                etype,
                tgt_ntype,
            )
...
def get_neg_path_score_func(g, weight, exclude_node=[]):
    ...
    u, v = g.edges()
    neg_path_score_map = {
        edge: log_in_degrees[edge[1]] - log_eweights[i]
...
def bidirectional_dijkstra(
    g, src_nid, tgt_nid, weight=None, ignore_nodes=None, ignore_edges=None
):
    ...
    while not prev_path or len(listA) < k:
        if not prev_path:
            length, path = bidirectional_dijkstra(
                g, src_nid, tgt_nid, weight, ignore_nodes_init, ignore_edges_init
            )
...
def get_comp_g_edge_labels(comp_g, edge_labels):
    ...
    for can_etype in edge_labels:
        ...
        start_comp_g_max_nid, end_comp_g_max_nid = (
            ntypes_to_comp_g_max_nids[start_ntype],
        ...
        comp_g_eids = comp_g.edge_ids(
            comp_g_start_nids.tolist(), comp_g_end_nids.tolist(), etype=etype
        )
...
def get_comp_g_path_labels(comp_g, path_labels):
    ...
    for path in path_labels:
        ...
        for can_etype, start_nid, end_nid in path:
            ...
            comp_g_start_nid = ntype_to_tensor_nids_to_comp_g_nids[start_ntype][
                start_nid
            ].item()
            comp_g_end_nid = ntype_to_tensor_nids_to_comp_g_nids[end_ntype][
                end_nid
            ].item()

            comp_g_eid = comp_g.edge_ids(
                comp_g_start_nid, comp_g_end_nid, etype=can_etype
            )
...
def plot_hetero_graph(
    ...
    if selected_node_dict is not None:
        selected_node_dict = {
            ntype: list(selected_node_dict[ntype]) for ntype in selected_node_dict
        }
    ...
    if layout == "spring":
        ...
        if ntypes_to_nlayers is not None:
            ntype_ids_to_nlayers = {
                ghetero.get_ntype_id(ntype): ntypes_to_nlayers[ntype]
                for ntype in ghetero.ntypes
            }
        else:
            ntype_ids_to_nlayers = {
                ghetero.get_ntype_id(ntype): i for i, ntype in enumerate(ghetero.ntypes)
            }
    ...
    for ntype in ghetero.ntypes:
        ...
        node_shape = (
            ntypes_to_nshapes[ntype] if ntypes_to_nshapes else default_node_shape
        )
        node_color = (
            ntypes_to_ncolors[ntype] if ntypes_to_ncolors else default_node_color
        )
        ...
        if selected_node_dict is not None:
            ...
            if curr_hetero_selected_nid is not None:
                ...
                for hetero_nid in curr_hetero_nids:
                    curr_node_color += [
                        selected_node_color
                        if hetero_nid in curr_hetero_selected_nid
                        else node_color
                    ]
    ...
    if selected_edge_dict is not None:
        ...
        nx.draw_networkx_edges(
            nx_graph, pos, homo_selected_edge_list, **selected_edge_kwargs, ax=ax
        )
    ...
    if label == "none":
        ...
        if ghetero.ndata.get(label):
            homo_nids_to_hetero_ndata_feat = get_homo_nids_to_hetero_ntype_data_feat(
                ghetero, label
            )
            if label_offset:
...
def get_homo_nids_to_hetero_ntype_data_feat(ghetero, feat=dgl.NID):
    ...
    for ntype in ghetero.ntypes:
        hetero_ndata_feat += [
            f"{ntype[0]}" + f"{feat}" for feat in ghetero.ndata[feat][ntype].tolist()
        ]
