...
def remove_edges_of_high_degree_nodes(ghomo, max_degree=10, always_preserve=[]):
    ...
    u, v = ghomo.edges()
    high_degree_edge_mask = torch.isin(u, high_degree_nids) | torch.isin(
        v, high_degree_nids
    )
...
class PaGELink(nn.Module):
    ...
    def __init__(
        self, model, lr=0.001, num_epochs=100, alpha=1.0, beta=1.0, args=None, log=False
    ):
    ...
    def _prune_graph(self, ghetero, prune_max_degree=-1, k_core=2, always_preserve=[]):
        ...
        if prune_max_degree > 0:
            max_degree_pruned_ghomo = remove_edges_of_high_degree_nodes(
                ghomo, prune_max_degree, always_preserve
            )
            k_core_ghomo = remove_edges_except_k_core_graph(
                max_degree_pruned_ghomo, k_core, always_preserve
            )
        ...
        for etype in ghetero.canonical_etypes:
            ...
            pruned_ghetero_eid_mask = pruned_ghomo_eid_mask[
                cum_num_edges : cum_num_edges + num_edges
            ]
            etypes_to_pruned_ghetero_eid_masks[etype] = pruned_ghetero_eid_mask

            remove_ghetero_eids = (
                (~pruned_ghetero_eid_mask).nonzero().view(-1).to(device)
            )
            pruned_ghetero = dgl.remove_edges(
                pruned_ghetero, eids=remove_ghetero_eids, etype=etype
            )
    ...
    def path_loss(self, src_nid, tgt_nid, g, eweights, num_paths=5):
        ...
        paths = k_shortest_paths_with_max_length(
            g, src_nid, tgt_nid, weight=neg_path_score_func, k=num_paths
        )
    ...
    def get_edge_mask(
        ...
        ntype_hetero_nids_to_homo_nids = get_ntype_hetero_nids_to_homo_nids(
            ghetero, debug=self.debug
        )
        ...
        if prune_graph:
            ...
            ml_ghetero, etypes_to_pruned_ghetero_eid_masks = self._prune_graph(
                ghetero, prune_max_degree, k_core, [homo_src_nid, homo_tgt_nid]
            )
        else:
        ...
        eweight_norm = 0
        EPS = 1e-3
        # print(f'start train') # for debug
        for e in range(self.num_epochs):
            # Apply sigmoid to edge_mask to get eweight
            ml_eweight_dict = {
                etype: ml_edge_mask_dict[etype].sigmoid() for etype in ml_edge_mask_dict
            }
            ...
            if with_path_loss:
                path_loss = self.path_loss(
                    homo_src_nid, homo_tgt_nid, ml_ghomo, ml_ghomo_eweights
                )
            else:
                path_loss = 0
    ...
    def get_paths(
        self, src_nid, tgt_nid, ghetero, edge_mask_dict, num_paths=1, max_path_length=3
    ):
        ...
        neg_path_score_func = get_neg_path_score_func(
            ghomo, "eweight", [src_nid.item(), tgt_nid.item()]
        )
        homo_paths = k_shortest_paths_with_max_length(
            ghomo,
            homo_src_nid,
            homo_tgt_nid,
        ...
        if len(homo_paths) > 0:
            for homo_path in homo_paths:
                ...
                for i in range(1, len(homo_path)):
                    ...
                    hetero_u_ntype, hetero_u_nid = homo_nids_to_ntype_hetero_nids[
                        homo_u
                    ]
                    hetero_v_ntype, hetero_v_nid = homo_nids_to_ntype_hetero_nids[
                        homo_v
                    ]
                    can_etype = ntype_pairs_to_cannonical_etypes[
                        (hetero_u_ntype, hetero_v_ntype)
                    ]
    ...
    def explain(
        ...
        (comp_g_src_nid, comp_g_tgt_nid, comp_g, comp_g_feat_nids) = (
            hetero_src_tgt_khop_in_subgraph(
                self.src_ntype, src_nid, self.tgt_ntype, tgt_nid, ghetero, num_hops
            )
        )
        ...
        comp_g_edge_mask_dict = self.get_edge_mask(
            comp_g_src_nid,
            comp_g_tgt_nid,
            comp_g,
            comp_g_feat_nids,
