...
g, processed_g, pred_pair_to_edge_labels, pred_pair_to_path_labels = load_dataset(
    args.dataset_dir, args.dataset_name, args.valid_ratio, args.test_ratio
)
mp_g, train_pos_g, train_neg_g, val_pos_g, val_neg_g, test_pos_g, test_neg_g = [
    g for g in processed_g
]
...
state = torch.load(
    f"{args.saved_model_dir}/{args.saved_model_name}.pth", map_location="cpu"
)
...
for i in tqdm(test_ids):
    ...
    with torch.no_grad():
        pred = (
            model(comp_g_src_nid, comp_g_tgt_nid, comp_g, comp_g_feat_nids)
            .sigmoid()
            .item()
            > 0.5
        )
...
for explainer in args.eval_explainer_names:
    saved_explanation_mask = (
        f"{explainer}_{args.saved_model_name}_pred_edge_to_comp_g_edge_mask"
    )
...
if args.eval_path_hit:
    ...
    for explainer in args.eval_explainer_names:
        ...
        for src_tgt in comp_graphs:
            ...
            comp_g_edge_mask_dict = pred_edge_to_comp_g_edge_mask[src_tgt]
            topk_to_path_hit = eval_edge_mask_topk_path_hit(
                comp_g_edge_mask_dict, comp_g_path_labels, topks
            )
