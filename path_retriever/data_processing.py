...
def process_data(
    ...
    eids = torch.arange(M)
    train_pos_eids, val_pos_eids, test_pos_eids = eids_split(
        eids, val_ratio, test_ratio
    )
    ...
    if neg == "pred_etype_neg":
        ...
        adj = sp.coo_matrix(
            (np.ones(len(u)), (u.numpy(), v.numpy())), shape=(src_N, tgt_N)
        )
        ...
        adj = sp.coo_matrix(
            (np.ones(len(src_tgt_u)), (src_tgt_u.numpy(), src_tgt_v.numpy())),
            shape=(src_N, tgt_N),
        )
        adj_neg = 1 - adj.todense()
        neg_u, neg_v = np.where(adj_neg != 0)
    ...
    train_neg_eids, val_neg_eids, test_neg_eids = eids_split(
        torch.from_numpy(neg_eids), val_ratio, test_ratio
    )
    ...
    train_neg_u, train_neg_v = (
        np.take(neg_u, train_neg_eids),
        np.take(neg_v, train_neg_eids),
    )
    val_neg_u, val_neg_v = np.take(neg_u, val_neg_eids), np.take(neg_v, val_neg_eids)
    test_neg_u, test_neg_v = (
        np.take(neg_u, test_neg_eids),
        np.take(neg_v, test_neg_eids),
    )
    ...
    train_pos_g = dgl.heterograph(
        {pred_can_etype: (train_pos_u, train_pos_v)}, num_nodes_dict
    )
    train_neg_g = dgl.heterograph(
        {pred_can_etype: (train_neg_u, train_neg_v)}, num_nodes_dict
    )
    val_pos_g = dgl.heterograph(
        {pred_can_etype: (val_pos_u, val_pos_v)}, num_nodes_dict
    )
    val_neg_g = dgl.heterograph(
        {pred_can_etype: (val_neg_u, val_neg_v)}, num_nodes_dict
    )
    test_pos_g = dgl.heterograph(
        {pred_can_etype: (test_pos_u, test_pos_v)}, num_nodes_dict
    )

    test_neg_g = dgl.heterograph(
        {pred_can_etype: (test_neg_u, test_neg_v)}, num_nodes_dict
    )

    mp_g = remove_all_edges_of_etype(
        g, pred_etype
    )  # Remove pred_etype edges but keep nodes
    return mp_g, train_pos_g, train_neg_g, val_pos_g, val_neg_g, test_pos_g, test_neg_g


def load_dataset(
    dataset_dir, dataset_name, split, val_ratio, test_ratio, stage, debug=False
):
    """
    Parameters
    ----------
    ...
    pred_etype = "likes"
    neg = "src_tgt_neg"
    processed_g = process_data(
        g, val_ratio, test_ratio, src_ntype, tgt_ntype, pred_etype, neg
    )
    if debug:
    ...
    if stage == "step1":
        ...
        test_pos_g = dgl.heterograph(
            {(src_ntype, pred_etype, tgt_ntype): (src, dst)},
