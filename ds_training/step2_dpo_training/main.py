...
def parse_args():
    ...
    ## add sft_loss for stable training
    parser.add_argument(
        "--add_sft", action="store_true", help="add_sft loss for training"
    )
    ## Tokenizer
    parser.add_argument(
        "--add_eot_token",
...
def main():
    ...
    model = create_hf_model(
        AutoModelForCausalLM, args.model_name_or_path, tokenizer, ds_config
    )
    ...
    if ref_zero_stage != 3:
        ...
        ref_zero_stage = 0
    ref_ds_config = get_eval_ds_config(args.offload_reference_model, ref_zero_stage)
    ...
    ref_ds_eval_config = get_eval_ds_config(offload=False, stage=ref_zero_stage)
    ref_ds_eval_config["train_micro_batch_size_per_gpu"] = (
        args.per_device_train_batch_size
    )
    ref_ds_eval_config["train_batch_size"] = (
        args.per_device_train_batch_size
        * torch.distributed.get_world_size()
        * args.gradient_accumulation_steps
    )
    ref_model = create_hf_model(
        AutoModelForCausalLM, args.model_name_or_path, tokenizer, ref_ds_eval_config
    )
    ...
    def evaluation(model, ref_model, tokenizer, eval_dataloader):
        ...
        for step, batch in enumerate(eval_dataloader):
            ...
            for i in range(batch_size):
                divergence_ind = (
                    (chosen_input_ids[i] != rejected_input_ids[i]).nonzero().squeeze(-1)
                )
            ...
            ref_logps = get_batch_logps(
                ref_outputs.logits, batch["input_ids"], label_mask
            )
            ...
            loss = (
                -torch.nn.functional.logsigmoid(logits) * (1 - args.label_smoothing)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # exit()
            if args.add_sft:
                sft_loss = -get_batch_logps(
                    outputs.logits[:batch_size],
                    batch["input_ids"][:batch_size],
                    label_mask[:batch_size],
                ).mean()
