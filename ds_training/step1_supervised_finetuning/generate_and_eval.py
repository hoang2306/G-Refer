...
def main(
    ...
    if save_dir is None:
        save_dir = (
            f"{model_path}/output_{model.dtype}_bs{batch_size}/{args.streategy}/msgm"
        )
    ...
    for lang in langs:
        ...
        if args.streategy == "Parallel":
            if lang == "En_gsm8k":
                ...
                with open(
                    f"{gsm8k_test_jsonl}/mgsm_{lang}.json", "r", encoding="utf-8"
                ) as f:
                    gsm8k_datas = [json.loads(line) for line in f]
        else:
            if lang == "En_gsm8k":
                ...
                with open(
                    f"{gsm8k_test_jsonl}/mgsm_English.json", "r", encoding="utf-8"
                ) as f:
                    gsm8k_datas = [json.loads(line) for line in f]
        ...
        for i in tqdm(range(start_index, len(gsm8k_datas), batch_size)):
            ...
            input_str_list, output_str_list = gsm8k_batch_gen(
                lang, [d["query"] for d in cur_gsm8k_batch], batch_llama
            )
    ...
    with open(
        Path(save_dir) / f"MSGM_evaluate_results_bs{batch_size}.csv", "w", newline=""
    ) as file:
...
def gsm8k_batch_gen(lang_, gsm8k_questions, batch_llm):
