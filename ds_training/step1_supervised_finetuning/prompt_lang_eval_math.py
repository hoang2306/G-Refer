...
def prompt_eval(
    args, model_baseline, model_fintuned, tokenizer, device, prompts, labels, lang
):
    all_count = 0
    count = 0
    index = 0
...
def main():
    ...
    model_fintuned = create_hf_model(
        AutoModelForCausalLM, args.model_name_or_path_finetune, tokenizer, bf16=True
    )
    ...
    with open(
        Path(args.model_name_or_path_finetune)
        / f"MSGM_evaluate_results_bs{args.batch_size}.csv",
        "w",
        newline="",
    ) as file:
